...
export class FairChainComponent implements OnInit {
   ...
   *
   * initializes the different options available for nodes and edges.
  ...
  /**
   * Checks if ChangingEdge is set to Label, edits the edge correctly.
   *
  ...
  /**
   * Switch the "add Edge"-button's color and addEdge functionality
  ...
  /**
   * Adds a group, based on the selected color and the given name.
   * If there exists already a group with the given name, it will opne a snackbar telling so to the user.
   * Otherwise it adds the new group in the groupsService and updates the options
   */
  public addGroup(): void {
    if (this.groupsServices.doesGroupExist(this.groupInfo.name)) {
      ...
      //updating the network's options
      this.options.groups = this.groupsServices.getGroups(this.defaultGroupColor);
  ...
  /**
   * Switch the "add Node"-button's color and addNode functionality
   * on or off if the button is pressed
   */
   ...
   * 
   * This impacts all nodes having the selected group
   */
  ...
  public changeNodeGroupColor() {
    ...
    //updates group's setting in the network
    this.network.setOptions(this.options);

    //updates the color for each node, based on their group
  ...
  /**
   * Switch changesNode between DeleteNodeFlag and None, if the button is pressed
   */
  ...
  /**
   * Opens a snackbar to ask the user if he wants to add a logo node
   */
