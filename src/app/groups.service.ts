 ...
 * the generate groups with complex syntax without letting the program crash, as the real group name is stored
 * with another value ("group"n""). Here is also declared a short method to check if the added group name already
 * exists, if so, the snack bar service is called and outputs a snackbar below to give a feedback at
 * the user.
 */
...
export class GroupsService {
  ...
  public doesGroupExist(groupName: string): boolean {
    return this.groups.map((g: Group) => {
      return g.name;
    }).includes(groupName);
  }

  public getGroups(defaultColor: string) {
    let temp: string[] = this.groups.map((g: Group) => {
      return '\"' + g.visJsName + '\" : { \"color\" : \"' + g.color + '\"}';
    });
  ...
  public getGroupsName(): string[] {
    let out: string[] = this.groups.map((g: Group) => {
      return g.name;
    });
    out.splice(0, 0, 'none');
    return out;
  ...
  public findVisJsName(groupName: string) {
    ...
    return this.groups.find((g: Group) => {
  ...
  public setGroups(groups: Group[]) {
    for (let i = 0; i < groups.length; i++) {
      groups[i].visJsName = 'group' + i;
    }
    this.groups = groups;
  }

  public setGroupColor(name: string, color: string) {
    this.groups.find((g: Group) => {
      return g.name === name;
    }).color = color;
  }
